...
def deploy():
    ...
    with cd("%s" % path):
        ...
        run('su %s -c "source %s/venv/bin/activate && pip install -r requirements.txt"' % (env.appuser, path))
        print(green("Collecting static files..."))
        run('su %s -c "source %s/venv/bin/activate && python manage.py collectstatic --noinput"' % (env.appuser, path))
        # print(green("Creating the database..."))
        # run("sudo -u postgres createdb %s" % dbsettings['NAME'])
        print(green("Syncing the database..."))
        run('su %s -c "source %s/venv/bin/activate && python manage.py syncdb"' % (env.appuser, path))
